...
verify observable behavior, ownership, and failure recovery independently of
implementation details.

## Test infrastructure

Meson owns executable isolation, parallelism, deadlines, selection, repetition and JUnit
logs. C tests use one always-active `CHECK` macro: it evaluates its condition once and
...
Python's standard-library `unittest` owns system/PTY fixtures and assertions; Meson owns
their outer process lifecycle. Keep the C assertion layer small; add a test library only
when fixture reuse or diagnostics justify another dependency.
...
specific category and, at the executable boundary, the exit status. Failure diagnostics
identify the input, expected result, and observed outcome. Use Unicode escapes in
...
allocation failure before emitting a completion event and preserve earlier bindings.
Named launch scenarios cover first/later fork and registration failure, stopped
unregistered children and termination before exec. These substitutions remain outside
production interfaces. They are not exhaustive OS or allocator fault coverage.

Syntax tests bound recursive parsing even for invalid nested declarations, before AST
validation can run, and verify that parsed source survives destruction of the input.
...
`process`, `platform`, `fault` and `pty`; later stages add `stream` and `codec`. The
...
claim raw-editor, full Unicode width, stream or codec coverage before those features
exist.
...
Planned libFuzzer targets cover lexer/parser, pattern matching, JSON conversion, command
argument encoding, input decoding/paste, grapheme segmentation, layout, edit/undo
sequences, and a bounded pure evaluator. Targets must not launch processes or touch user
files. Reset state per input, bound evaluation steps and allocations, and preserve
crashing inputs as regressions. Use ASan/UBSan and the matching Clang fuzzer runtime.
Keep target functions independent of the fuzzing driver.
...
The eight `tests/benchmarks/*.rill` workloads cover direct tail calls, curried calls,
structural equality, text comparison, `map`/`sum` materialization, closure construction,
shared DAGs, and large Records. Each checks its result and launches no external
programs. Five C benchmarks isolate syntax/code acquisition, explicit collection,
binding snapshots, retained captures, and differently shared DAGs. Run all thirteen
separately from the correctness gate:

```sh
meson test -C build/release --benchmark --repeat=7 --print-errorlogs
```

Meson runs benchmarks serially. For before/after comparisons, retain both release
executables, warm each once, and alternate their execution order on identical scripts.
Report median and range, including shell startup and prelude loading. Keep raw timing
logs in ignored build directories. Do not mix release results with sanitizer timings or
infer allocator traffic or cache misses from elapsed time. The C syntax benchmark runs
64 entries of 16,384 Unit statements through the production parser and evaluator. The C
GC benchmark retains 100,000 two-edge objects, then allocates and reclaims 100,000
leaves in each of 50 rounds. It disables automatic triggers for that experiment and
prints retained bytes, summed collection time, and maximum observed collection time
using the monotonic clock. The C binding benchmark builds 4,096 adapter bindings,
commits a shadowing entry, and looks up every name, repeating eight times; it
deliberately excludes same-scope parser/pattern checks. The C capture benchmark retains
10,000 closures with eight captures each, reports reachable object count and tracked
bytes, and times 50 full collections over that graph. The sharing benchmark builds two
96-level, width-32 ternary DAGs with equal unfoldings and different transitions, then
compares them 16 times. It exercises equivalence classes beyond matching-shape DAGs.
These pauses describe that graph and process, not a worst-case latency guarantee. Build
an unchanged copy of each C workload against both source snapshots with identical
compiler options for before/after evidence.

Runtime regressions check ownership independently of timing: rooted partial builders,
deep/shared/cyclic GC graphs, bounded tail-call state, closure/code lifetime, immutable
snapshots, prepared-pattern error order, and Record indexing. Equality cases verify
unsupported leaves despite identity or earlier mismatches, different DAG sharing,
height-aware depth limits, and allocation failure. Fault sweeps run preparation and
publication under stress GC. Passing benchmark results supplement these regressions;
wall-clock speed is never a correctness gate.
...
The [implementation plan](implementation-plan.md) owns the four delivery stages and maps
them to these contracts. Each stage includes its relevant tests and failure paths;
