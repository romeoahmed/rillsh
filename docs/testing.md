...
`FD_SETSIZE` boundary when permitted. A controlling-PTY case inherits enough descriptors
...
record-rest matching, error conversion, sorting, Unicode scalar splitting, exports, and
...
failure guarantee under stress GC. Module fixtures reject non-regular files without
...
| `lines-fuzz` | At most 128 KiB of bytes; line count/content against a byte scan, CRLF trimming, final fragments, UTF-8 errors, and stream cleanup |
...
and expected line values. JSON system tests compare emitted data against Python values,
...
build/fuzz/tests/json-fuzz build/fuzz/json-corpus tests/fuzz/corpus/json \
...
The ten `tests/benchmarks/*.rill` workloads cover direct tail calls, curried calls,
structural equality, text comparison, `map`/`sum` materialization, closure construction,
shared DAGs, large Records, immutable Record updates, and checkpoint unwinding. Each
...
repeated constants, and wide/deep JSON decoding. Run all 23 separately from the
correctness gate:
...
| --- | --- |
| Syntax, bindings, snapshots, layouts | Meson process duration, including the work and validation in the executable |
| GC and captures | Explicit collection timed separately from graph construction |
| Shared equality | Differently shaped DAGs with equal unfoldings; verify the comparison result |
| Persistence | Scoped-handle traversal with an unrelated live Stream, preventing a trivial fast path |
...
| JSON | Wide and deep conversion measured separately from explicit collection |
