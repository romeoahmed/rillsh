...
[interaction](interaction.md), and [platform](platform.md) remain the behavior contracts;
[development](development.md) owns the toolchain and [testing](testing.md) owns evidence.
The layout describes the target repository. Add directories and interfaces as their
implementations become necessary.

## Repository layout

```text
meson.build                   Project, native build options, dependencies, install rules
meson.options                 Only options actually needed, such as opt-in fuzz targets
.clang-format
.clang-tidy
.gitignore
src/
  meson.build                 Explicit source lists and private build targets
  main.c                      CLI parsing and session entry/exit
  ...
  editor/                     Decoder, edit/undo state, syntax integration, layout, rendering
...
bench/                        Release workloads and result schema; run outputs stay ignored
...
`tools/` contains the Unicode generator/verifier, standard-library source embedding,
and the small check driver shared by developers and CI. These use Python's standard
library. `tests/` contains `unit/` for C component tests, `spec/` for Rill conformance
cases, `system/` for process/PTY scenarios, `helpers/` for controlled C child programs,
and `fuzz/` for bounded fuzz entry points. Test fixtures stay with the cases using them.
...
completion/help metadata, never an evaluator or a language closure to invoke.
`platform` contains mechanisms and native differences; it does not decide Rill job
success, stream escape, or REPL publication policy.
...
separate: components return structured information; the session formats and emits it.
A value printer lives with the language adapter code and emits text/style spans, so the
...
opaque Job/Stream references. `library/process.c` translates an evaluated plan into an
...
reports at the same adapter boundary. Exec objects retain no Values; language-side
state stays rooted in the runtime/adapter.
...
resource services it uses. A suspended native operation retains owned state and a
stable registration token, not a pointer to a temporary C stack frame. A required
callback contract is declared by its consumer; the session supplies the implementation.
No lower component includes the session interface, re-enters the event loop, or polls
a global singleton.
...
supervision; `editor/input.c`, `edit.c`, `layout.c`, and `render.c` separate testable state
machines. These examples are starting points, not an exhaustive file checklist.
...
Boundary declarations carry concise Doxygen contracts under the
[comment conventions](development.md#comments-and-api-documentation). Document ownership
and failure behavior there; keep implementation invariants in ordinary local comments.
...
Maintain one list of boundary headers for the optional `api-docs` target. Documentation
CI requires it; ordinary shell builds do not. Cross-component includes must follow the
...
| Authored `stdlib/*.rill`         | Embedded C module text in the build tree | Planned Meson dependency, rebuilt with source changes |
...
generation, and test helpers.
Implement the production source/diagnostic path, initial Values/GC and continuation
protocol, command parsing/lowering, and the POSIX supervisor with its byte pumps.
Support literal commands, byte pipelines, redirection, launch reports, cancellation,
and external job control.
Use the same unary native-call path that later language features will extend. Provide
CLI/script input and a canonical-input prompt; text/style separation and RGB capability
policy start here even though cursor-addressed editing comes later.
...
modules, and transactional REPL binding publication. Complete code/capture ownership,
GC roots, and collection of shadowed bindings. Bootstrap bundled modules and the prelude;
implement pure sequence/text operations and the public JobPlan/process API through the
existing adapters. Extend canonical multiline input to the complete grammar and add startup config.

**Boundary.** The language and reusable plan model are complete. Structured streaming,
stream-dependent lifetime cases, and suspended mixed evaluations belong to stage 3;
the editor still uses the plain input path.
...
source/transform/sink resumption connected to the existing byte pumps, codecs,
collection/sorting budgets, and the filesystem/JSON library. Complete `capture`, `stream`,
`through`, and checked completion. Integrate cancellation and suspended evaluator/job
contexts with foreground resume, session snapshots, and shutdown. Complete all library
contracts and render results incrementally through text/style spans on the existing prompt.

**Boundary.** This is the complete language and execution model usable through scripts
and plain interaction. Rich editing, persistent history, and completion UI remain stage 4.
There is still only one active user evaluator; no background callback scheduler is added.
...
parser-aware submission/highlighting, layout/rendering, fixed key set, completion worker,
history persistence/search, and coordinated notifications. Integrate these into the
existing terminal owner and loop. Finish help, examples, API reference review,
installation, release workloads, fuzz regression corpora, and the complete Linux/macOS
validation matrix.

**Boundary.** Deliver the specified first release. Add no new language model, editor mode
framework, worker-thread runtime, terminal database, or compatibility backend during
...
regressions at subsequent gates. Record exact commands, build identities, results, and
remaining scope at each checkpoint. Maintain a coverage map from contract IDs to real
tests, including partial/pending cases until their stage completes. The stage-4 matrix
must close every remaining coverage gap.
