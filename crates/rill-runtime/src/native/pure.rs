...
pub fn apply<'gc>(mc: &Mutation<'gc>, argument: Value<'gc>) -> Result<Value<'gc>, Error> {
    ...
    match (name.as_str(), arguments) {
        ...
        ("aggregation_key", [key @ Value::String(_)]) => Ok(*key),
        ("aggregation_key", _) => Err(Error::type_error("aggregation key requires String")),
        ...
        ("help", [value]) => Ok(Value::string(mc, value.help())),
        ("string", [value]) => text::convert(mc, *value),
